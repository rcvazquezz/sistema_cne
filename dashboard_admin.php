            ...
            <section class="section hidden p-4 md:p-6" id="seccion-respaldos">
                <div class="max-w-4xl mx-auto w-full">
                    ...
                    <div class="bg-white rounded-xl p-4 md:p-6 shadow mb-6">
                        ...
                        <p class="text-xs text-gray-500 mb-3">Con la sesión de administrador abierta, el panel consulta al servidor cada 30 segundos; el servidor (zona America/Caracas) acepta el respaldo durante todo el minuto programado (p. ej. 20:04:00–20:04:59). La configuración se guarda en <code class="text-xs bg-gray-100 px-1 rounded">configuracion_sistema</code> bajo la clave <code class="text-xs bg-gray-100 px-1 rounded">respaldo_automatico</code> (JSON).</p>
        ...
        function cargarHistorialBackup() {
            fetch('ajax/admin_backup_controller.php?action=get_historial')
                ...
                .then(d => {
                    ...
                    if (!list.length) {
                        tbody.innerHTML = '<tr><td colspan="4" class="px-4 py-4 text-center text-gray-500">Sin respaldos registrados</td></tr>';
                        return;
                    }
                    list.forEach(h => {
                ...
                .catch(() => {
                    const tbody = document.getElementById('tabla-backup-historial');
                    if (tbody) tbody.innerHTML = '<tr><td colspan="4" class="px-4 py-4 text-center text-red-500">No se pudo cargar el historial</td></tr>';
                });
        }
